...
local tmpnet = torch.load(actionParam.folders.output .. actionParam.partname .. ".model")
local tmpx = tmpnet:getParameters()
x1:copy(tmpx)
tmpnet = nil
tmpx = nil
...
feval2 = function(x2_new)
    ...
    local input = train.LR[_nidx2_]

    -- reset gradients (gradients are always accumulated, to accommodate batch methods)
    dl_dx2:zero()

    -- evaluate the loss function and its derivative wrt x, for that sample
    local intermediatePic = net1:forward(input)
    local loss_x2 = criterion:forward(net2:forward(intermediatePic), target)
    net2:backward(intermediatePic, criterion:backward(net2.output, target))
...
for i = 1, epochs do
    ...
    loss_logger:add{train_loss2, validation_loss2}
    if saveInterval and i % saveInterval == 0 then
        loss_logger:plot()  
    end

    -- Print to console
		print('i'..i..'\tloss2 = ' .. train_loss2 .. '\tgrad2 norm = ' .. current_abs_grad2)
    if analysisInterval and i % analysisInterval == 0 then
